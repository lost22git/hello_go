...
func TestLogSlog(t *testing.T) {
	...
	// Json
	var jsonhandler = slog.NewJSONHandler(os.Stdout, &slog.HandlerOptions{AddSource: true, Level: logLevel})
	var jsonlogger = slog.New(jsonhandler).With("handler", "Json")
	...
	// Text
	var texthandler = slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{AddSource: true, Level: logLevel})
	var textlogger = slog.New(texthandler).With("handler", "Text")
