...
func TestStrcut(t *testing.T) {
	...
	var a2 = Account2{}
	assert.EqualValues(t, 4+4+16, unsafe.Sizeof(a2))
	assert.EqualValues(t, 8, unsafe.Alignof(a2))
	assert.EqualValues(t, 0, unsafe.Offsetof(a2.Id))
	assert.EqualValues(t, 8, unsafe.Offsetof(a2.Name))
	assert.EqualValues(t, 4, unsafe.Offsetof(a2.Balance))
}
