...
func TestHttpGet(t *testing.T) {
	...
	var request, err = http.NewRequest("GET", "https://httpbin.org/status/800", bytes.NewReader([]byte("")))
	if err != nil {
...
func TestHttpPost(t *testing.T) {
	var client = &http.Client{Timeout: 2 * time.Second}
	var request, err = http.NewRequest("POST", "https://httpbin.org/post", bytes.NewReader([]byte(`{"msg":"hello go"}`)))
	if err != nil {
		assert.Failf(t, "Failed to new http request", "err=%v", err)
	}
